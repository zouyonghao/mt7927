...
/*******************************************************************************
*                    E X T E R N A L   R E F E R E N C E N C E S
********************************************************************************
*/
#include "precomp.h"
#include "mt6639.h"
				...
				u_int8_t fgAssertRst)
{
	...
	DBGLOG(INIT, INFO, "MT6639: mt6639HalCbInfraRguWfRst called with fgAssertRst=%d\n", fgAssertRst);
	...
	if (fgAssertRst) {
		...
		DBGLOG(INIT, INFO, "MT6639: Read cr_bus_rst 0x%x: 0x%x\n", u4AddrVal, u4CrVal);
		u4CrVal |= BIT(4);
		HAL_MCR_WR(prAdapter, u4AddrVal, u4CrVal);
		DBGLOG(INIT, INFO, "MT6639: Write assert cr_bus_rst 0x%x[4]: 0x%x\n", u4AddrVal, u4CrVal);
	} else {
		/*e.1*/
		u4AddrVal = CBTOP_RGU_BASE;
		HAL_MCR_RD(prAdapter, u4AddrVal, &u4CrVal);
		DBGLOG(INIT, INFO, "MT6639: E. Read 0x%x: 0x%x\n", u4AddrVal, u4CrVal);

		u4CrVal &= ~BIT(4);

		HAL_MCR_WR(prAdapter, u4AddrVal, u4CrVal);
		DBGLOG(INIT, INFO, "MT6639: Write de-assert cr_bus_rst 0x%x[4]: 0x%x\n", u4AddrVal, u4CrVal);
	}
...
#define MCU_IDLE	0x1D1E
	...
	DBGLOG(INIT, INFO, "MT6639: mt6639HalPollWfsysSwInitDone called\n");

	/* Polling until WF WM is done */
	while (TRUE) {
		HAL_MCR_RD(prAdapter, WF_TOP_CFG_ON_ROMCODE_INDEX_REMAP_ADDR,
			&u4CrValue);
		DBGLOG(INIT, INFO, "MT6639: Poll ROM_INDEX, 0X81021604: 0x%x\n", u4CrValue);
		if (u4CrValue == MMIO_READ_FAIL)
			DBGLOG(INIT, INFO, "MT6639: MMIO read CR fail\n");
		else if (u4CrValue == MCU_IDLE) {
			DBGLOG(INIT, INFO, "MT6639: MCU_IDLE detected, breaking\n");
			break;
		}

		if (u4ResetTimeCnt >= u4ResetTimeTmout) {
			DBGLOG(INIT, INFO, "MT6639: Poll Sw Init Done FAIL\n");
			fgSwInitDone = FALSE;
			break;
		}
	...
	DBGLOG(INIT, INFO, "MT6639: mt6639HalPollWfsysSwInitDone returning %d\n", fgSwInitDone);
	return fgSwInitDone;
}
